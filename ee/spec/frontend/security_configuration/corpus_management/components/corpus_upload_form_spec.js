...
describe('Corpus upload modal', () => {
  ...
  describe('corpus modal', () => {
    describe('initial state', () => {
      ...
      it('emits the "reset-corpus" event when a file is uploaded', () => {
        const file = new File(['foo'], 'foo.txt');
        const input = findFileInput();
        Object.defineProperty(input.element, 'files', { value: [file] });
        input.trigger('change');

        expect(wrapper.emitted('reset-corpus')).toHaveLength(1);
      });
    });
    ...
    describe('file selected state', () => {
      ...
      it('emits the "begin-file-upload" event when the upload file button is clicked', () => {
        findUploadCorpus().trigger('click');
        expect(wrapper.emitted('begin-file-upload')).toHaveLength(1);
      });
    });
    ...
    describe('uploading state', () => {
      ...
      it('shows the choose file button as disabled', () => {
        expect(findUploadAttachment().exists()).toBe(true);
        expect(findUploadAttachment().attributes('aria-disabled')).toBe('true');
      });
      ...
      describe('clicking cancel button', () => {
        it('emits the reset corpus event', () => {
          ...
          expect(wrapper.emitted()['reset-corpus']).toEqual([[]]);
        });
      });
    });
    ...
    describe('error states', () => {
      ...
      describe('blank corpus name', () => {
        ...
        it('shows the upload corpus button as disabled', () => {
          expect(findUploadCorpus().exists()).toBe(true);
          expect(findUploadCorpus().attributes('aria-disabled')).toBe('true');
        });
